...
<body>
    ...
    <div class="tm-hero d-flex justify-content-center align-items-center" data-parallax="scroll" data-image-src="{{ CRUDBOoster::publicPath('img/bg2.jpg') }}">
        <form class="d-flex tm-search-form" method='post' action="/public/cctv/grid/{{ $owner }}/{{ $status }}/{{ $publik }}">
            <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
            <input class="form-control tm-search-input" type="search" placeholder="Cari" aria-label="Cari" name="cari" id="cari" value="{{ $keyword }}">
            ...
            @foreach($kegiatan as $keg)
                ...
                <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 col-12 mb-5">
                    <figure class="effect-ming tm-video-item">
                        <iframe id="ifrm-{{ $keg->name }}" frameBorder="0" src="https://pantaucctv.online/player/index.html?{{ $keg->id }}" allowfullscreen allow="autoplay" scrolling="no" style="width:100%;height:300px"></iframe>

                    </figure>
                    <div class="d-flex justify-content-between tm-text-gray">
                        <span id="idv_stats"></span>
                    </div>
                </div>
            @endforeach
        </div> <!-- row -->
    ...
    <script>
        ...
        $('.tm-search-form').on('submit', function(e){
            e.preventDefault();
            var cari = $('#cari').val();
            // print(len($('#cari').val()))
            if(cari.length>0){
                window.location.href = "/public/cctv/grid/{!! $owner !!}/{!! $status !!}/{!! $publik !!}/"+ cari;
            }else{
                window.location.href = "/public/cctv/grid/{!! $owner !!}/{!! $status !!}/{!! $publik !!}/all";
            }
        })
